...
import { FC, JSXElementConstructor, Key, ReactElement, ReactFragment, ReactPortal, forwardRef, useEffect, useRef, useState } from 'react'
...
const Header: FC<{ pathname: string }> = ({ pathname }) => {
  ...
  function handleMenuToggle() {
    (menu_toggle.current as unknown as HTMLElement).classList.toggle("expanded")
  }
  return (
      ...
      <div className="container-fluid flex items-center justify-between">
        ...
        <div className="flex items-center h-auto pl-[10px] md:pl-[20px] xl:py-0 py-[20px]">
          <a href="#">
            ...
            <span className='text-white lg:text-[25px] text-[22px] font-bold'>Forex4You</span>
          </a>
        </div>
        {/* Mobile Nav */}
        ...
        <ul className="flex items-center">
          ...
          <a href="https://dashboard.timelineinvestment.space/sign-up.php" id=""
            className="text-[15px] text-black bg-custom-yellow px-[25px] py-[17px] md:inline-block hidden hover:bg-white hover:text-custom-dark-primary custom-transition ml-[35px]">Create
            Account</a>
        </ul>
      </div>
    </header>
    ...
    <div ref={ref} className="mobile-nav-wrapper xl:hidden">
      ...
      <nav className="mobile-nav-content">
        ...
        <div className='mb-[30px] flex'>
          <a href="#">
            {/* <img src={logo} width="140" alt="" /> */}
            <span className='text-white text-[22px] font-bold inline-block'>Forex4You</span>
          </a>
        </div>
        {/*  */}
        <a className='w-full text-center mx-0 mb-[20px] text-black flex items-center bg-custom-yellow justify-center h-[45px]' href="#">Login</a>
        {/*  */}
        <div>
          <ul className='m-0 p-0'>
            {links.map((link: { href: string | undefined; text: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | ReactFragment | ReactPortal | null | undefined }, index: Key | null | undefined) => (
              <li onClick={() => (ref.current as HTMLElement).classList.toggle('expanded')} className='border-b border-b-[rgba(255,255,255,.1)]' key={index}>
                <a className='flex justify-between leading-[30px] text-text-light font-medium h-[50px] items-center' href={link.href}>
                  {link.text}
                  <button className='w-[30px] h-[30px] p-0 border-none bg-custom-yellow text-black flex items-center justify-center text-center'>
                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24"><path fill="currentColor" d="M8.025 22L6.25 20.225L14.475 12L6.25 3.775L8.025 2l10 10l-10 10Z" /></svg>
                  </button>
                </a>
              </li>
            ))}
        ...
        <ul className='mx-0 my-[20px]'>
          <li className='text-[14px] relative flex items-center text-[#a2a2b1] font-medium'>
            <i className='text-white bg-dark-secondary w-[32px] h-[32px] text-center rounded-[50%] mr-[10px] flex items-center justify-center icon-email'>

            </i>
            <a href="mailto:support@Forex4You.com" className='text-white'>support@Forex4You.com</a>
          </li>
          <li className='text-[14px] relative flex items-center text-[#a2a2b1] font-medium mt-[15px]'>
            <i className='text-white bg-dark-secondary w-[32px] h-[32px] text-center rounded-[50%] mr-[10px] flex items-center justify-center icon-telephone'>
            </i>
  ...
  return (
    <ul className=" xl:flex items-center hidden">
      {links.map((link, index) => (
        <li key={index} className={`text-text-primary py-[30px] text-[15px] ${index != 0 && 'ml-[35px]'}`}>
